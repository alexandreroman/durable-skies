...
The backend has four workflow types:
...
- **OrderWorkflow** (one per submitted order, short-lived): thin per-order state machine that forwards the order to `FleetWorkflow.submit_order` and waits for a `delivery_done` signal before exiting. Exists so every order is individually queryable / cancellable in the Temporal UI; it does NOT own queue state (the fleet does).
